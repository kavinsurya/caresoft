...
def create_user_db():
    try:
        ...
        c.execute("""CREATE TABLE users
                 (username, password, name, roll, email)""")
        conn.commit()
    except Exception as e:
        print('Error: ', e)

def register_user(item, password):
    try:
        conn = sqlite3.connect(USER_DB)
        c = conn.cursor()
        c.execute('insert into users(username, password) values(?,?)', (item, password))
        conn.commit()
        return {"u": item, "status": NOTSTARTED}
    except Exception as e:
        print('Error: ', e)
        return None

def login(item, password):
    try:
        conn = sqlite3.connect(USER_DB)
        c = conn.cursor()
        c.execute("select * from users where username=? and password=?", (item, password))
        status = c.fetchall()
        print(status)
        return status
    except Exception as e:
